...
- **[Architecture/Infrastructure/JSON] Découplage de données statiques & Interface TopicContent :**
    ...
    - **TopicPage & Intégration du Hook** : Simplification de `TopicPage` (`src/pages/Topic/index.tsx`) pour utiliser exclusivement le hook `useTopicFetcher` de manière réactive et propre, avec des optimisations `useMemo` qui empêchent tout re-rendering inutile ou avertissement ESLint de cascade.
