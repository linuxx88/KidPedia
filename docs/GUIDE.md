...
L'architecture a atteint un niveau de maturité industrielle. Le démantèlement des monolithes, la migration vers une réactivité pure et le durcissement total des types garantissent une base de code stable et inclusive.Persistance : La migration IndexedDB (v3.35.0) est récente
...
1. **Smart Store / Dumb UI :** Le store gère la donnée brute, le composant ou le hook métier gère l'intention de jeu, l'UI gère le texte.Persistance : La migration IndexedDB (v3.35.0) est récente
